// Pure JavaScript QR Code SVG Generator (Byte Mode)
...
QRCode.prototype = {
    ...
    setupTypeInfo: function(test, maskPattern) {
        ...
        for (var j = 0; j < 15; j++) {
            var mod2 = (!test && ((bits >> j) & 1) === 1);
            if (j < 8) {
                this.modules[8][this.moduleCount - j - 1] = mod2;
            } else if (j < 9) {
                this.modules[8][15 - j - 1 + 1] = mod2;
            } else {
                this.modules[8][15 - j - 1] = mod2;
            }
        }
...
QRCode.createBytes = function(buffer, rsBlocks) {
    ...
    for (var r = 0; r < rsBlocks.length; r++) {
        ...
        for (var k = 0; k < ecdata[r].length; k++) {
            var modIndex = k + modPoly.getLength() - ecdata[r].length;
            ecdata[r][k] = (modIndex >= 0) ? modPoly.get(modIndex) : 0;
        }
    }
    ...
    for (var n = 0; n < maxDcCount; n++) {
        for (var r1 = 0; r1 < rsBlocks.length; r1++) {
            if (n < dcdata[r1].length) {
                data[index++] = dcdata[r1][n];
            }
        }
    }
    for (var p = 0; p < maxEcCount; p++) {
        for (var r2 = 0; r2 < rsBlocks.length; r2++) {
            if (p < ecdata[r2].length) {
                data[index++] = ecdata[r2][p];
            }
        }
    }
    return data;
...
for (var j = 8; j < 256; j++) QRMath.EXP_TABLE[j] = QRMath.EXP_TABLE[j - 4] ^ QRMath.EXP_TABLE[j - 5] ^ QRMath.EXP_TABLE[j - 8] ^ QRMath.EXP_TABLE[j - 6];
...
QRRSBlock.getRSBlocks = function(typeNumber, errorCorrectionLevel) {
    ...
    if (rsBlock == null) throw new Error("bad rs block @ typeNumber:" + typeNumber + "/errorCorrectionLevel:" + errorCorrectionLevel);
...
export function getQRCodeMatrix(text) {
    ...
    if (text.length > 120) typeNum = 8;
    const qr = new QRCode(typeNum, QRErrorCorrectionLevel.M);
    qr.addData(text);
    qr.make();
